...
function InstallBox({ copied, onCopy }: InstallBoxProps) {
  ...
  return (
    <div
      id="install"
      className="mt-8 inline-flex items-center gap-3.5 border border-border bg-code px-[18px] py-3 font-mono text-sm"
    >
...
function HeroCopy() {
  ...
  return (
    <div className="min-w-[380px] flex-1">
      <h1 className="m-0 font-mono text-[44px] font-bold leading-[1.15] text-text [text-wrap:pretty]">
        {t("landing.hero.titleLine1")}
        <br />
        {t("landing.hero.titleLine2")}
        <span className="ml-2 inline-block h-[0.9em] w-[0.5em] animate-blink bg-accent" />
      </h1>
      <p className="mt-[22px] max-w-[44ch] text-[18px] text-muted [text-wrap:pretty]">
        {t("landing.hero.body")}
      </p>
...
export function Hero() {
  return (
    <section className="mx-auto flex max-w-[1080px] flex-wrap items-center gap-14 px-8 pt-[72px] pb-20">
      <HeroCopy />
      <div className="min-w-[420px] flex-[1.1]">
        <TerminalMockup />
      </div>
    </section>
  );
